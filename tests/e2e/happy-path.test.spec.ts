    ...
    { tag: ['@happy-path'] }, async () => {
        ...
        test('E2E-001 - Create a new employee and give the new user a leave it should reflect with the user when they sign in',
            async ({ page, loginPage, dashboardPage, pimPage, navBar }) => {
                ...
                test.step('Add a new employee', async () => {
                    await pimPage.clickAddBtn();
                    await pimPage.isCardTitleVisible();
                    test.step('Fill the user name', async () => {
                    ...
                    test.step('Create login details for the new employee', async () => {
                        test.step('Enable the create login details toggle', async () => {
                            await pimPage.setCreateLoginDetails(true);
                        })
                        test.step('Fill the login details', async () => {
                            await pimPage.createLoginDetails(
                                new_employee.credentials.username,
                                new_employee.credentials.password,
                                new_employee.credentials.password
                            );
                        })
